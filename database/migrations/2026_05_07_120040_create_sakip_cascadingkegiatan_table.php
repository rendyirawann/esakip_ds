<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    public function up(): void
    {
        Schema::create('sakip_cascadingkegiatan', function (Blueprint $table) {
            $table->bigIncrements('refcascadingkegiatan_id');
            $table->bigInteger('refcascadingprogram_id')->nullable();
            $table->bigInteger('refsasaranrenstra_id')->nullable();
            $table->bigInteger('refindikatorsasaranrenstra_id')->nullable();
            $table->bigInteger('refprogram_id')->nullable();
            $table->bigInteger('refkegiatan_id')->nullable();
            $table->text('uraian_sasarankegiatan');
            $table->text('uraian_indikatorkegiatan');
            $table->bigInteger('refperiode_id')->nullable();
            $table->bigInteger('refskpd_id')->nullable();
            $table->text('kegiatan_target')->nullable();
            $table->text('kegiatan_satuan')->nullable();
            $table->timestamps();
        });
    }

    public function down(): void
    {
        Schema::dropIfExists('sakip_cascadingkegiatan');
    }
};