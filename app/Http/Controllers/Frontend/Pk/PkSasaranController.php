...
class PkSasaranController extends Controller
{
    public function index(Request $request)
    {
        ...
        $skpd_id = $request->skpd_id ?? session('pk_sasaran_skpd_id');
        $current_periode_id = $request->periode_id ?? session('pk_sasaran_periode_id');
    ...
    {
        $skpd_id = $request->skpd_id ?? session('pk_sasaran_skpd_id');
        $periode_id = $request->periode_id ?? session('pk_sasaran_periode_id');

        if ($request->has('skpd_id') && $request->has('periode_id') && $request->skpd_id != null) {
            session([
                'pk_sasaran_skpd_id' => $request->skpd_id,
                'pk_sasaran_periode_id' => $request->periode_id
            ]);
