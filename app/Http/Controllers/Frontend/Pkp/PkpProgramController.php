...
class PkpProgramController extends Controller
{
    public function index(Request $request)
    {
        ...
        $skpd_id = $request->skpd_id ?? session('pkp_program_skpd_id');
        $current_periode_id = $request->periode_id ?? session('pkp_program_periode_id');
        ...
        return view('frontend.pkp.program.index', compact(
    ...
    {
        $skpd_id = $request->skpd_id ?? session('pkp_program_skpd_id');
        $periode_id = $request->periode_id ?? session('pkp_program_periode_id');

        if ($request->has('skpd_id') && $request->has('periode_id') && $request->skpd_id != null) {
            session([
                'pkp_program_skpd_id' => $request->skpd_id,
                'pkp_program_periode_id' => $request->periode_id
            ]);
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function($row) {
                return '<button type="button" class="btn btn-icon btn-sm btn-light-primary" onclick="editPkp('.$row->refindikatorprogram_id.')">
    ...
    public function store(Request $request)
    {
        ...
        if ($model) {
            ...
            $model->save();
            return response()->json(['success' => true, 'message' => 'Data PK Perubahan Program berhasil disimpan!']);
        }
