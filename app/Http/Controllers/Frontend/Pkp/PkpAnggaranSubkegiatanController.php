...
class PkpAnggaranSubkegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('frontend.pkp.anggaran.subkegiatan', compact(
    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('anggaran_pk', function($row) {
                $anggaran = $row->anggaran_pk ?? 0;
                return 'Rp. ' . number_format($anggaran, 0, ',', '.');
            })
            ->addColumn('anggaran_pkp_input', function($row) {
                $val = $row->anggaran_pk_p ?? 0;
                return '<div class="input-group input-group-sm w-150px ms-auto">
                            <span class="input-group-text bg-light">Rp</span>
                            <input type="text" class="form-control text-end input-anggaran-pkp bg-light" 
    ...
    public function update(Request $request)
    {
        ...
        if ($model) {
            $model->anggaran_pk_p = $anggaran;
            $model->save();
            return response()->json(['success' => true, 'message' => 'Anggaran PK Perubahan berhasil disimpan!']);
        }
