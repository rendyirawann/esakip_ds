...
class PkpTriwulanKegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('frontend.pkp.triwulan.kegiatan', compact('skpds', 'periodes', 'isSuperadmin', 'current_skpd', 'current_periode', 'current_triwulan'));
    }
    ...
    public function data(Request $request)
    {
        ...
        $query = SakipIndikatorcascadingkegiatanTriwulan::select(
                ...
                'sakip_indikatorcascadingkegiatan.target_pk_p', 
        ...
        return DataTables::of($query)
            ->addIndexColumn()
            ->addColumn('nama_kegiatan', function($row) { return $row->nama_kegiatan ?? '-'; })
            ->addColumn('indikator', function($row) { return $row->uraian_indikatorkegiatan ?? '-'; })
            ->addColumn('satuan', function($row) { return $row->kegiatan_satuan ?? '-'; })
            ...
            ->addColumn('action', function($row) {
                return '<button type="button" class="btn btn-icon btn-light-primary btn-sm btn-edit" data-id="'.$row->refindikatorkegiatantriwulan_id.'"><i class="ki-outline ki-pencil fs-2"></i></button>';
            })
            ->rawColumns(['action'])
            ->make(true);
    }
    ...
    public function store(Request $request)
    {
        ...
        $model->save();
        return response()->json(['success' => true, 'message' => 'Data PK Perubahan Triwulan Kegiatan berhasil disimpan!']);
    }
}
