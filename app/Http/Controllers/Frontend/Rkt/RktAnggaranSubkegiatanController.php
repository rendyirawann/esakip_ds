...
class RktAnggaranSubkegiatanController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('frontend.rkt.anggaran.subkegiatan', compact(
    ...
    public function data(Request $request)
    {
        ...
        return DataTables::of($query)
            ...
            ->addColumn('anggaran_renstra', function($row) {
                // Ambil anggaran dari tabel cascading subkegiatan
                $anggaran = $row->cascading->subkegiatan_anggaran ?? 0;
                return 'Rp. ' . number_format($anggaran, 0, ',', '.');
            })
            ->addColumn('anggaran_rkt_input', function($row) {
                $val = $row->anggaran_rkt ?? 0;
                return '<div class="input-group input-group-sm w-150px ms-auto">
                            <span class="input-group-text bg-light">Rp</span>
                            <input type="text" class="form-control text-end input-anggaran-rkt bg-light" 
