...
class RealisasiSubkegiatanTriwulanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $skpd_id    = $isSuperadmin ? $request->session()->get('capaian_subkegiatan_tw_skpd_id') : ($user->refskpd_id ?? null);
        $periode_id = $request->session()->get('capaian_subkegiatan_tw_periode_id');
        $current_triwulan = $request->session()->get('capaian_subkegiatan_tw_triwulan', 1);
        ...
        return view('frontend.capaian.realisasi-subkegiatan.triwulan', compact(
    ...
    public function data(Request $request)
    {
        ...
        if ($isSuperadmin) {
            $request->session()->put('capaian_subkegiatan_tw_skpd_id', $skpd_id);
        }
        $request->session()->put('capaian_subkegiatan_tw_periode_id', $periode_id);
        $request->session()->put('capaian_subkegiatan_tw_triwulan', $triwulan_id);
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function($r) {
                return '<button type="button" class="btn btn-icon btn-light-primary btn-sm btn-edit"
                            data-id="'.$r->refindikatorsubkegiatantriwulan_id.'" title="Input Realisasi">
    ...
    {
        $data = SakipIndikatorcascadingsubkegiatanTriwulan::select(
                'sakip_indikatorcascadingsubkegiatan_triwulan.*',
                'sakip_indikatorcascadingsubkegiatan.target_pk',
                'sakip_cascadingsubkegiatan.uraian_indikatorsubkegiatan'
            )
            ->join('sakip_indikatorcascadingsubkegiatan', 'sakip_indikatorcascadingsubkegiatan_triwulan.refindikatorsubkegiatan_id', '=', 'sakip_indikatorcascadingsubkegiatan.refindikatorsubkegiatan_id')
            ->join('sakip_cascadingsubkegiatan', 'sakip_indikatorcascadingsubkegiatan.refcascadingsubkegiatan_id', '=', 'sakip_cascadingsubkegiatan.refcascadingsubkegiatan_id')
            ->where('sakip_indikatorcascadingsubkegiatan_triwulan.refindikatorsubkegiatantriwulan_id', $id)
            ->firstOrFail();
