...
class RealisasiProgramTriwulanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $skpd_id    = $isSuperadmin ? $request->session()->get('capaian_program_tw_skpd_id') : ($user->refskpd_id ?? null);
        $periode_id = $request->session()->get('capaian_program_tw_periode_id');
        $current_triwulan = $request->session()->get('capaian_program_tw_triwulan', 1);
        ...
        return view('frontend.capaian.realisasi-program.triwulan', compact(
    ...
    public function data(Request $request)
    {
        ...
        if ($isSuperadmin) {
            $request->session()->put('capaian_program_tw_skpd_id', $skpd_id);
        }
        $request->session()->put('capaian_program_tw_periode_id', $periode_id);
        $request->session()->put('capaian_program_tw_triwulan', $triwulan_id);

        $query = SakipIndikatorcascadingprogramTriwulan::select(
                'sakip_indikatorcascadingprogram_triwulan.*',
                'sakip_indikatorcascadingprogram.target_pk',
        ...
        return DataTables::of($query)
            ...
            ->addColumn('action', function($r) {
                return '<button type="button" class="btn btn-icon btn-light-primary btn-sm btn-edit"
                            data-id="'.$r->refindikatorprogramtriwulan_id.'" title="Input Realisasi">
    ...
    {
        $data = SakipIndikatorcascadingprogramTriwulan::select(
                'sakip_indikatorcascadingprogram_triwulan.*',
                'sakip_indikatorcascadingprogram.target_pk',
                'sakip_cascadingprogram.uraian_indikatorprogram'
            )
            ->join('sakip_indikatorcascadingprogram', 'sakip_indikatorcascadingprogram_triwulan.refindikatorprogram_id', '=', 'sakip_indikatorcascadingprogram.refindikatorprogram_id')
            ->join('sakip_cascadingprogram', 'sakip_indikatorcascadingprogram.refcascadingprogram_id', '=', 'sakip_cascadingprogram.refcascadingprogram_id')
            ->where('sakip_indikatorcascadingprogram_triwulan.refindikatorprogramtriwulan_id', $id)
            ->firstOrFail();
