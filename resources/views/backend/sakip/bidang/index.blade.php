...
@section('title', 'Manajemen Bidang SAKIP')
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            ...
            <div class="page-title d-flex flex-column justify-content-center me-3">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-gray-900">Manajemen Bidang</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="card-header border-0 pt-6">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-user-table-filter="search" class="form-control form-control-solid w-250px ps-13" placeholder="Cari Bidang..." />
            </div>
        </div>
        <div class="card-toolbar">
            <div class="d-flex justify-content-end">
                <button type="button" class="btn btn-primary" onclick="addData()">
                    <i class="ki-outline ki-plus fs-2"></i> Tambah Bidang
                </button>
            </div>
        </div>
    </div>
    <div class="card-body py-4">
        <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_bidang">
            <thead>
                <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                    <th class="min-w-50px">No</th>
                    <th class="min-w-100px">Urusan</th>
                    <th class="min-w-100px">Kode Bidang</th>
                    <th class="min-w-150px">Nama Bidang</th>
...
@section('modals')
<!-- Modal -->
<div class="modal fade modal-right" id="modal_bidang" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="form_bidang" class="ajax-form" action="javascript:void(0)">
                @csrf
                <input type="hidden" name="id" id="bidang_id">
                <div class="modal-header">
                    <h2 class="fw-bold" id="modal_title">Tambah Bidang</h2>
                ...
                <div class="modal-body py-10 px-lg-17">
                    <div class="fv-row mb-7">
                        ...
                        <select name="refurusan_id" id="refurusan_id" class="form-select form-select-solid" data-control="select2" data-placeholder="Pilih Urusan..." data-dropdown-parent="#modal_bidang" required>
                            <option></option>
                    ...
                    <div class="fv-row mb-7">
                        <label class="required fs-6 fw-semibold mb-2">Kode Bidang</label>
                        <input type="text" class="form-control form-control-solid" name="kode_bidang" id="kode_bidang" required />
                    </div>
                    <div class="fv-row mb-7">
                        <label class="required fs-6 fw-semibold mb-2">Nama Bidang</label>
                        <input type="text" class="form-control form-control-solid" name="nama_bidang" id="nama_bidang" required />
                    </div>
                    <div class="fv-row mb-7">
...
<script>
    ...
    $(function () {
        ...
        $('#form_bidang').submit(function (e) {
            e.preventDefault();
            var formData = $(this).serialize();
            const btn = $('#btn_submit');
            btn.attr('data-kt-indicator', 'on').prop('disabled', true);
            $.ajax({
                url: "{{ route('sakip.bidang.store') }}",
                type: "POST",
                data: formData,
    ...
    function addData() {
        ...
        $('#modal_title').text('Tambah Bidang');
        $('#modal_bidang').modal('show');
    }

    function editData(id) {
        $.get("{{ url('/admin/sakip/bidang') }}/" + id + "/edit", function (data) {
            $('#bidang_id').val(data.refbidang_id);
            $('#kode_bidang').val(data.kode_bidang);
            $('#nama_bidang').val(data.nama_bidang);
            $('#refurusan_id').val(data.refurusan_id).trigger('change');
            $('#is_aktif').prop('checked', data.bidang_isaktif == 1);
            $('#modal_title').text('Edit Bidang');
            $('#modal_bidang').modal('show');
        });
    }
