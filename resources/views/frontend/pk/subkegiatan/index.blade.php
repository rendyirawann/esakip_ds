...
@section('title', 'Target PK Indikator Sub Kegiatan (Tahunan)')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Indikator Sub Kegiatan (Tahunan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Indikator Sub Kegiatan (Tahunan)</li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <table id="kt_datatable_pk" class="table align-middle table-row-dashed fs-6 gy-5">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-center">Target<br>Renstra</th>
                                <th class="text-center">Target<br>RKT</th>
                                <th class="text-center">Keterangan<br>RKT</th>
                                <th class="text-center">Target<br>PK</th>
                                <th class="text-center">Keterangan<br>PK</th>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Update Target PK Sub Kegiatan</h2>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#form_edit_pk').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('frontend.pk.subkegiatan.store') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
    ...
    function initDataTable() {
        dt = $('#kt_datatable_pk').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.pk.subkegiatan.data') }}",
                data: function(d) {
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group bg-light"><td colspan="9" class="fw-bolder text-gray-800 fs-6 py-3">Sub Kegiatan: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
    ...
    function editPk(id) {
        $.ajax({
            url: "{{ url('frontend/pk/subkegiatan') }}/" + id + "/edit",
            type: "GET",
            success: function(data) {
                $('#edit_id').val(data.refindikatorsubkegiatan_id);
                $('#edit_indikator').val(data.cascading ? data.cascading.uraian_indikatorsubkegiatan : '-');
