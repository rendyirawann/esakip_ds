...
@section('title', 'Target PK Indikator Kegiatan (Tahunan)')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Indikator Kegiatan (Tahunan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Indikator Kegiatan (Tahunan)</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Update Target PK Kegiatan</h2>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#form_edit_pk').submit(function(e) {
            ...
            $.ajax({
                url: "{{ route('frontend.pk.kegiatan.store') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
    ...
    function initDataTable() {
        dt = $('#kt_datatable_pk').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.pk.kegiatan.data') }}",
                data: function(d) {
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group bg-light"><td colspan="9" class="fw-bolder text-gray-800 fs-6 py-3">Kegiatan: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
    ...
    function editPk(id) {
        $.ajax({
            url: "{{ url('frontend/pk/kegiatan') }}/" + id + "/edit",
            type: "GET",
            success: function(data) {
                $('#edit_id').val(data.refindikatorkegiatan_id);
                $('#edit_indikator').val(data.cascading ? data.cascading.uraian_indikatorkegiatan : '-');
