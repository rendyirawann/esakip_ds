...
@section('title', 'Anggaran Sub Kegiatan PK Perubahan')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Anggaran Sub Kegiatan PK Perubahan</h1>
                    ...
                    <table id="kt_datatable_anggaran" class="table align-middle table-row-dashed fs-6 gy-5">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-end">Anggaran PK</th>
                                <th class="text-end min-w-200px">Anggaran PK P</th>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '.btn-edit-anggaran', function() {
            ...
            const input = $(`.input-anggaran-pkp[data-id="${id}"]`);
        ...
        $(document).on('click', '.btn-save-anggaran', function() {
            ...
            $.ajax({
                url: "{{ route('frontend.pkp.anggaran-subkegiatan.update') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: id,
    ...
    function initDataTable() {
        dt = $('#kt_datatable_anggaran').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.pkp.anggaran-subkegiatan.data') }}",
                data: function(d) {
            ...
            columns: [
                ...
                { data: 'anggaran_pk', name: 'anggaran_pk', className: 'text-end text-dark' },
                { data: 'anggaran_pkp_input', name: 'anggaran_pkp_input', className: 'text-end', orderable: false, searchable: false },
                { 
                    data: null, 
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group bg-light"><td colspan="8" class="fw-bolder text-gray-800 fs-6 py-3">Kegiatan: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
