...
@section('title', 'Target PK Perubahan Indikator Kegiatan (Tahunan)')
@section('content')
<div class="d-flex flex-column flex-column-fluid">
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6"><div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack"><div class="page-title d-flex flex-column justify-content-center flex-wrap me-3"><h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Perubahan Indikator Kegiatan (Tahunan)</h1><ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1"><li class="breadcrumb-item text-muted"><a href="{{ route('frontend.dashboard') }}" class="text-muted text-hover-primary">Dashboard</a></li><li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li><li class="breadcrumb-item text-muted">PK Perubahan</li><li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li><li class="breadcrumb-item text-dark">Indikator Kegiatan (Tahunan)</li></ul></div></div></div>
...
<div class="modal fade" id="modal_edit_pkp" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered mw-650px"><div class="modal-content">
    <div class="modal-header"><h2 class="fw-bold">Update Target PK Perubahan Kegiatan</h2><div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal"><i class="ki-outline ki-cross fs-1"></i></div></div>
    <div class="modal-body py-10 px-lg-17"><form id="form_edit_pkp" class="no-loader"><input type="hidden" id="edit_id">
        <div class="fv-row mb-7"><label class="fs-6 fw-semibold mb-2">Indikator Kegiatan</label><input type="text" id="edit_indikator" class="form-control form-control-solid" readonly /></div>
        <div class="fv-row mb-7"><label class="fs-6 fw-semibold mb-2">Kegiatan</label><input type="text" id="edit_kegiatan" class="form-control form-control-solid" readonly /></div>
...
$(document).ready(function() {
    ...
    $('#form_edit_pkp').submit(function(e) { e.preventDefault(); const btn=$('#btn_save_pkp'); btn.attr('data-kt-indicator','on').prop('disabled',true);
        $.ajax({ url:"{{ route('frontend.pkp.kegiatan.store') }}",type:"POST",data:{_token:"{{ csrf_token() }}",id:$('#edit_id').val(),target_pk_p:$('#edit_target_pk_p').val(),keterangan_pk_p:$('#edit_keterangan_pk_p').val()},
...
function initDataTable() {
    dt = $('#kt_datatable_pkp').DataTable({ processing:false,serverSide:true,pageLength:25,
        ajax:{url:"{{ route('frontend.pkp.kegiatan.data') }}",data:function(d){d.skpd_id=$('#filter_skpd_id').val();d.periode_id=$('#filter_periode_id').val();}},
        columns:[
        ...
        drawCallback:function(){var api=this.api(),rows=api.rows({page:'current'}).nodes(),last=null;api.column(1,{page:'current'}).data().each(function(g,i){if(last!==g){$(rows).eq(i).before('<tr class="group bg-light"><td colspan="11" class="fw-bolder text-gray-800 fs-6 py-3">Kegiatan: '+g+'</td></tr>');last=g;}});}
    });
}
function editPkp(id) {
    $.ajax({url:"{{ url('frontend/pkp/kegiatan') }}/"+id+"/edit",type:"GET",success:function(d){
        $('#edit_id').val(d.refindikatorkegiatan_id);$('#edit_indikator').val(d.cascading?d.cascading.uraian_indikatorkegiatan:'-');$('#edit_kegiatan').val(d.program?d.program.nama_kegiatan:'-');$('#edit_target_pk_p').val(d.target_pk_p);$('#edit_keterangan_pk_p').val(d.keterangan_pk_p);$('#modal_edit_pkp').modal('show');
    }});
}
</script>
@endpush
