...
@section('title', 'Target PK Perubahan Triwulan Indikator Kegiatan')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Perubahan Triwulan Indikator Kegiatan</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Triwulan Kegiatan</li>
                </ul>
            </div>
        </div>
    </div>
                            ...
                            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                                <div class="card-title">
                                    <h3 class="card-title fw-bold text-gray-800 fs-4 m-0" id="table_title">
                                        Data PK Triwulan Indikator Kegiatan
                                    </h3>
                                </div>
                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered mw-600px">
        <div class="modal-content">
            <div class="modal-header bg-light-primary py-4">
                <h2 class="fw-bold m-0" id="modal_title">Edit PK Triwulan Kegiatan</h2>
...
<script>
    ...
    $(document).ready(function() {
        ...
        function updateTableTitle() {
            ...
            $('#table_title').text(`Data PK Triwulan Indikator Kegiatan (Periode ${periodeText}) - Triwulan ${tw}`);
        }

        $(document).on('click', '.btn-edit', function() {
            const id = $(this).data('id');
            const tw = $('#active_triwulan').val();
            $('#modal_title').text(`Edit Target PK Triwulan ${tw} Kegiatan`);
            ...
            $.get("{{ url('frontend/pkp/kegiatan-triwulan') }}/" + id + "/edit", function(res) {
        ...
        $('#form_edit').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('frontend.pkp.kegiatan.triwulan.store') }}",
                type: "POST",
                data: $(this).serialize(),
    ...
    function initDataTable() {
        dt = $('#kt_datatable').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.pkp.kegiatan.triwulan.data') }}",
                data: function(d) {
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group"><td colspan="7" class="fw-bolder text-gray-800 fs-6 py-3 px-3 bg-light">Kegiatan: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
