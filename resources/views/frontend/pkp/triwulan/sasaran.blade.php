...
@section('title', 'Target PK Perubahan Triwulan Indikator Sasaran')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Perubahan Triwulan Indikator Sasaran</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Triwulan Sasaran</li>
                </ul>
            </div>
        </div>
    </div>
                            ...
                            <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                                <div class="card-title">
                                    <h3 class="card-title fw-bold text-gray-800 fs-4 m-0" id="table_title">
                                        Data PK Triwulan Indikator Sasaran
                                    </h3>
                                </div>
                            </div>
                                ...
                                <table id="kt_datatable" class="table align-middle table-row-dashed fs-6 gy-5 table-bordered">
                                    <thead>
                                        <tr class="text-start text-muted fw-bold fs-7 text-uppercase gs-0">
                                            ...
                                            <th class="text-center px-3">IKU</th>
                                            <th class="text-center px-3">PK Perubahan</th>
                                            <th class="text-center px-3">Target PKP<br/>Tahunan</th>
                                            <th class="text-center px-3" id="col_target_triwulan">Target PKP<br/>Triwulan 1</th>
    ...
    <div class="modal-dialog modal-dialog-centered mw-600px">
        <div class="modal-content">
            <div class="modal-header bg-light-primary py-4">
                <h2 class="fw-bold m-0" id="modal_title">Edit PK Triwulan</h2>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('.nav-link[data-bs-toggle="pill"]').on('shown.bs.tab', function (e) {
            ...
            $('#col_target_triwulan').html(`Target PKP<br/>Triwulan ${targetTriwulan}`);
        ...
        function updateTableTitle() {
            ...
            $('#table_title').text(`Data PK Triwulan Indikator Sasaran (Periode ${periodeText}) - Triwulan ${tw}`);
        }

        $(document).on('click', '.btn-edit', function() {
            const id = $(this).data('id');
            const tw = $('#active_triwulan').val();
            $('#modal_title').text(`Edit Target PK Triwulan ${tw}`);
            ...
            $.get("{{ url('frontend/pkp/sasaran-triwulan') }}/" + id + "/edit", function(res) {
        ...
        $('#form_edit').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('frontend.pkp.sasaran.triwulan.store') }}",
                type: "POST",
                data: $(this).serialize(),
    ...
    function initDataTable() {
        dt = $('#kt_datatable').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.pkp.sasaran.triwulan.data') }}",
                data: function(d) {
            ...
            columns: [
                ...
                { data: 'status_iku', name: 'status_iku', className: 'text-center px-3' },
                { data: 'status_pk', name: 'status_pk', className: 'text-center px-3' },
                { data: 'target_pkp_tahunan', name: 'target_pkp_tahunan', className: 'text-center px-3' },
                { data: 'target_pkp_triwulan', name: 'target_pkp_triwulan', className: 'text-center px-3' },
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group"><td colspan="9" class="fw-bolder text-gray-800 fs-6 py-3 px-3 bg-light">Sasaran: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
