...
@section('title', 'Target PK Perubahan Indikator Sasaran (Tahunan)')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target PK Perubahan Indikator Sasaran (Tahunan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">PK Perubahan</li>
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div id="table_card" class="card card-flush shadow-sm {{ $current_skpd && $current_periode ? '' : 'd-none' }}">
                ...
                <div class="card-body pt-0">
                    <table id="kt_datatable_pkp" class="table align-middle table-row-dashed fs-6 gy-5">
                        <thead><tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            ...
                            <th class="text-center">PK P</th>
                            <th class="text-center">Ket PKP</th>
...
<div class="modal fade" id="modal_edit_pkp" tabindex="-1" aria-hidden="true"><div class="modal-dialog modal-dialog-centered mw-650px"><div class="modal-content">
    <div class="modal-header"><h2 class="fw-bold">Update Target PK Perubahan</h2><div class="btn btn-icon btn-sm btn-active-icon-primary" data-bs-dismiss="modal"><i class="ki-outline ki-cross fs-1"></i></div></div>
    <div class="modal-body py-10 px-lg-17">
        <form id="form_edit_pkp" class="no-loader">
            <input type="hidden" id="edit_id">
            <div class="fv-row mb-7"><label class="fs-6 fw-semibold mb-2">Indikator</label><input type="text" id="edit_indikator" class="form-control form-control-solid" readonly /></div>
            <div class="fv-row mb-7"><label class="fs-6 fw-semibold mb-2">Sasaran</label><textarea id="edit_sasaran" class="form-control form-control-solid" rows="2" readonly></textarea></div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#btn_tampilkan').on('click', function() {
            const skpd = $('#filter_skpd_id').val(), periode = $('#filter_periode_id').val(), btn = $(this);
            if (!skpd || !periode) { Swal.fire({ text: "Silakan pilih SKPD dan Tahun terlebih dahulu.", icon: "warning" }); return; }
            btn.attr('data-kt-indicator', 'on').prop('disabled', true);
            $('#empty_state').addClass('d-none'); $('#table_card').removeClass('d-none');
        ...
        $('#form_edit_pkp').submit(function(e) {
            e.preventDefault();
            const btn = $('#btn_save_pkp');
            btn.attr('data-kt-indicator', 'on').prop('disabled', true);
            $.ajax({
                url: "{{ route('frontend.pkp.sasaran.store') }}", type: "POST",
                data: { _token: "{{ csrf_token() }}", id: $('#edit_id').val(), target_pk_p: $('#edit_target_pk_p').val(), keterangan_pk_p: $('#edit_keterangan_pk_p').val() },
    ...
    function initDataTable() {
        dt = $('#kt_datatable_pkp').DataTable({
            processing: false, serverSide: true, pageLength: 25,
            ajax: { url: "{{ route('frontend.pkp.sasaran.data') }}", data: function(d) { d.skpd_id = $('#filter_skpd_id').val(); d.periode_id = $('#filter_periode_id').val(); } },
            columns: [
                ...
                { data: 'iku_isaktif', name: 'iku_isaktif', className: 'text-center', render: function(d) { return d === 'T' ? '<span class="badge badge-light-success px-3">IKU</span>' : ''; } },
                { data: 'pk_isaktif', name: 'pk_isaktif', className: 'text-center', render: function(d) { return d === 'T' ? '<span class="badge badge-light-info px-3">PK</span>' : ''; } },
            ...
            drawCallback: function() {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) { $(rows).eq(i).before('<tr class="group bg-light"><td colspan="13" class="fw-bolder text-gray-800 fs-6 py-3">Sasaran: ' + group + '</td></tr>'); last = group; }
                });
            }
        });
    }
    function editPkp(id) {
        $.ajax({ url: "{{ url('frontend/pkp/sasaran') }}/" + id + "/edit", type: "GET",
            success: function(data) {
                ...
                $('#edit_target_pk_p').val(data.target_pk_p);
                $('#edit_keterangan_pk_p').val(data.keterangan_pk_p);
                $('#modal_edit_pkp').modal('show');
            }
        });
    }
</script>
