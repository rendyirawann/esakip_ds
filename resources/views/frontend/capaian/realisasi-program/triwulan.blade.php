...
@section('title', 'Capaian Kinerja - Realisasi Program (Triwulan)')
@section('content')
<div class="d-flex flex-column flex-column-fluid">
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Realisasi Program (Triwulan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-dark">Realisasi Program Triwulan</li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <div class="row g-5 align-items-end">
                        ...
                        <div class="col-md-2">
                            ...
                            <select id="filter_triwulan_id" class="form-select form-select-solid" data-control="select2">
                                ...
                                <option value="{{ $tw }}" {{ $current_triwulan == $tw ? 'selected' : '' }}>Triwulan {{ $tw }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="card shadow-sm">
                            <div class="card-body p-3">
                                <div class="d-flex flex-column gap-2" id="triwulan_tabs">
                                    @foreach([1,2,3,4] as $tw)
                                    <a href="javascript:void(0)" class="btn btn-sm fw-bold btn-tab-triwulan {{ $current_triwulan == $tw ? 'btn-primary' : 'btn-light-primary' }}" data-tw="{{ $tw }}">Triwulan {{ $tw }}</a>
                                    @endforeach
                                </div>
                            </div>
                        </div>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        $.ajax({
            url: "{{ url('frontend/capaian/realisasi-program/triwulan') }}/" + id + "/edit",
    ...
    $('#form_edit_capaian').submit(function(e) {
        ...
        btn.attr('data-kt-indicator', 'on').prop('disabled', true);
        $.ajax({
            url: "{{ route('frontend.capaian.realisasi-program.triwulan.store') }}",
            type: "POST",
            data: {
                _token: "{{ csrf_token() }}",
                id: $('[name="id"]').val(),
...
function initDataTable() {
    dt = $('#kt_datatable_capaian').DataTable({
        ...
        ajax: {
            url: "{{ route('frontend.capaian.realisasi-program.triwulan.data') }}",
            data: function(d) {
        ...
        drawCallback: function() {
            ...
            api.column(1, { page: 'current' }).data().each(function(group, i) {
                if (last !== group) {
                    $(rows).eq(i).before('<tr class="group bg-light"><td colspan="11" class="fw-bolder text-gray-800 fs-6 py-3 ps-4">Program: ' + group + '</td></tr>');
                    last = group;
                }
            });
        }
