...
@section('title', 'Capaian Kinerja - Realisasi Kegiatan (Tahunan)')
@section('content')
<div class="d-flex flex-column flex-column-fluid">
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Realisasi Kegiatan (Tahunan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-dark">Realisasi Kegiatan Tahunan</li>
                </ul>
            </div>
        </div>
    </div>
        ...
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div id="empty_state" class="card shadow-sm {{ $current_skpd && $current_periode ? 'd-none' : '' }}">
                <div class="card-body p-20 text-center">
                    ...
                    <p class="text-gray-400 fs-5">Silakan pilih SKPD dan tahun periode.</p>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-edit', function() {
        var id = $(this).data('id');
        $.ajax({ url: "{{ url('frontend/capaian/realisasi-kegiatan/tahunan') }}/" + id + "/edit", success: function(res) {
            ...
            $(document).off('input','#modal_realisasi').on('input','#modal_realisasi', function() {
                var t = parseFloat($('#modal_target_pk').val())||0, r = parseFloat($(this).val())||0;
                $('#modal_capaian').val(t > 0 && r >= 0 ? ((r/t)*100).toFixed(2) : '');
            });
        }});
    });
    $('#form_edit_capaian').submit(function(e) {
        e.preventDefault(); var btn = $('#btn_save_capaian');
        btn.attr('data-kt-indicator','on').prop('disabled',true);
        $.ajax({ url: "{{ route('frontend.capaian.realisasi-kegiatan.tahunan.store') }}", type: "POST",
...
function initDataTable() {
    dt = $('#kt_datatable_capaian').DataTable({ processing:false, serverSide:true, pageLength:25,
        ajax: { url: "{{ route('frontend.capaian.realisasi-kegiatan.tahunan.data') }}", data: function(d) {
        ...
        columns: [
            ...
            { data:'uraian_kegiatan', visible:false },
            { data:'indikator' },
            { data:'satuan', className:'text-center' },
            { data:'target_tahunan', className:'text-center' },
            { data:'target_pk', className:'text-center fw-bold text-primary' },
            { data:'realisasi_display', className:'text-center fw-bold' },
            { data:'capaian_display', className:'text-center', orderable:false },
            { data:'analisis', className:'fs-7' },
        ...
        drawCallback: function() {
            ...
            api.column(1,{page:'current'}).data().each(function(g,i) {
                if(last!==g) { $(rows).eq(i).before('<tr class="group bg-light"><td colspan="10" class="fw-bolder text-gray-800 fs-6 py-3 ps-4">Kegiatan: '+g+'</td></tr>'); last=g; }
            });
        }
    });
}
