...
@section('title', 'Capaian Kinerja - Realisasi Sub Kegiatan (Triwulan)')
@section('content')
<div class="d-flex flex-column flex-column-fluid">
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Realisasi Sub Kegiatan (Triwulan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-dark">Realisasi Sub Kegiatan Triwulan</li>
                </ul>
            </div>
        </div>
    </div>
                ...
                <div class="col-md-10"><div class="card card-flush shadow-sm">
                    ...
                    <div class="card-body pt-0"><table id="kt_datatable_capaian" class="table align-middle table-row-dashed fs-6 gy-5"><thead>
                        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                            <th class="w-50px text-center">No</th><th class="d-none">Sub Kegiatan</th><th class="min-w-200px">Indikator</th><th class="text-center">Satuan</th>
...
$(document).ready(function() {
    ...
    $(document).on('click','.btn-edit', function(){ var id=$(this).data('id');
        $.ajax({ url:"{{ url('frontend/capaian/realisasi-subkegiatan/triwulan') }}/"+id+"/edit", success: function(res){
    ...
    $('#form_edit_capaian').submit(function(e){ e.preventDefault(); var btn=$('#btn_save_capaian');
        ...
        $.ajax({ url:"{{ route('frontend.capaian.realisasi-subkegiatan.triwulan.store') }}", type:"POST",
...
function initDataTable(){
    dt=$('#kt_datatable_capaian').DataTable({processing:false,serverSide:true,pageLength:25,
        ajax:{url:"{{ route('frontend.capaian.realisasi-subkegiatan.triwulan.data') }}",data:function(d){
        ...
        drawCallback:function(){var api=this.api(),rows=api.rows({page:'current'}).nodes(),last=null;api.column(1,{page:'current'}).data().each(function(g,i){if(last!==g){$(rows).eq(i).before('<tr class="group bg-light"><td colspan="11" class="fw-bolder text-gray-800 fs-6 py-3 ps-4">Sub Kegiatan: '+g+'</td></tr>');last=g;}});}
    });
}
</script>
@endpush
