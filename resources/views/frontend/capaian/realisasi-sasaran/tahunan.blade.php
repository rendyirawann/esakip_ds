...
@section('title', 'Capaian Kinerja - Realisasi Sasaran (Tahunan)')
@section('content')
<div class="d-flex flex-column flex-column-fluid">
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Realisasi Sasaran (Tahunan)</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-dark">Realisasi Sasaran Tahunan</li>
                </ul>
            </div>
        </div>
    </div>
                    ...
                    <table id="kt_datatable_capaian" class="table align-middle table-row-dashed fs-6 gy-5">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="text-center">Target<br>Tahunan</th>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        $.ajax({
            url: "{{ url('frontend/capaian/realisasi-sasaran/tahunan') }}/" + id + "/edit",
            success: function(res) {
                $('#modal_form_content').html(res.html);
                $('#modal_edit_capaian').modal('show');
                // Auto-kalkulasi capaian dari target PK
    ...
    $('#form_edit_capaian').submit(function(e) {
        ...
        btn.attr('data-kt-indicator', 'on').prop('disabled', true);
        $.ajax({
            url: "{{ route('frontend.capaian.realisasi-sasaran.tahunan.store') }}",
            type: "POST",
...
function initDataTable() {
    dt = $('#kt_datatable_capaian').DataTable({
        ...
        ajax: {
            url: "{{ route('frontend.capaian.realisasi-sasaran.tahunan.data') }}",
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'target_rkt', name: 'target_rkt', className: 'text-center' },
