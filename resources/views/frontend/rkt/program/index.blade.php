...
@section('title', 'Target RKT Indikator Program')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target RKT Indikator Program</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Indikator Program</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div class="card mb-5 shadow-sm">
                <div class="card-body">
                    <form action="{{ route('frontend.rkt.program.filter') }}" method="POST" id="filter_form" class="no-loader">
                        @csrf
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Update Target RKT Program</h2>
            ...
            <div class="modal-body py-10 px-lg-17">
                <form id="form_edit_rkt" class="no-loader">
                    ...
                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">Indikator Program</label>
                    ...
                    <div class="fv-row mb-7">
                        ...
                        <input type="text" id="edit_program" class="form-control form-control-solid" readonly />
                    </div>
...
<script>
    ...
    function initDataTable() {
        dt = $('#kt_datatable_rkt').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.rkt.program.data') }}",
                data: function(d) {
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group bg-light"><td colspan="7" class="fw-bolder text-gray-800 fs-6 py-3">Program: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
    ...
    function editRkt(id) {
        $.ajax({
            url: "{{ url('frontend/rkt/program') }}/" + id + "/edit",
            type: "GET",
            success: function(data) {
                $('#edit_id').val(data.refindikatorprogram_id);
                
                let sasaran = data.cascading.uraian_sasaranprogram ?? '';
                let indikator = data.cascading.uraian_indikatorprogram ?? '';
                $('#edit_indikator').val((sasaran ? sasaran + ' / ' : '') + indikator);
                
                $('#edit_program').val(data.program ? data.program.nama_program : '-');
    ...
    $('#form_edit_rkt').submit(function(e) {
        ...
        $.ajax({
            url: "{{ url('frontend/rkt/program') }}/" + id,
            type: "PUT",
            data: {
                _token: "{{ csrf_token() }}",
