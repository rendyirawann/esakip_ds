...
@section('title', 'Target RKT Output Sub Kegiatan')
    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Target RKT Output Sub Kegiatan</h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-dark">Output Sub Kegiatan</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-xxl">
            ...
            <div class="card mb-5 shadow-sm">
                <div class="card-body">
                    <form action="{{ route('frontend.rkt.subkegiatan.filter') }}" method="POST" id="filter_form" class="no-loader">
                        @csrf
                    ...
                    <table id="kt_datatable_rkt" class="table align-middle table-row-dashed fs-6 gy-5">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">Indikator (Output)</th>
                                <th class="text-center">Satuan</th>
                                <th class="text-center">Target Renstra</th>
                                <th class="text-center">Target RKT</th>
                                <th>Keterangan</th>
    ...
    <div class="modal-dialog modal-dialog-centered mw-650px">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="fw-bold">Update Target RKT Sub Kegiatan</h2>
            ...
            <div class="modal-body py-10 px-lg-17">
                <form id="form_edit_rkt" class="no-loader">
                    ...
                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">Indikator Sub Kegiatan</label>
                        <textarea id="edit_indikator" class="form-control form-control-solid" rows="3" readonly></textarea>
                    </div>

                    <div class="fv-row mb-7">
                        <label class="fs-6 fw-semibold mb-2">Sub Kegiatan</label>
                        <input type="text" id="edit_subkegiatan" class="form-control form-control-solid" readonly />
                    </div>
...
<script>
    ...
    function initDataTable() {
        dt = $('#kt_datatable_rkt').DataTable({
            ...
            pageLength: 25,
            ajax: {
                url: "{{ route('frontend.rkt.subkegiatan.data') }}",
                data: function(d) {
            ...
            columns: [
                ...
                { data: 'target_renstra', name: 'target_renstra', className: 'text-center fw-bold' },
                { 
                    data: 'target_rkt', 
                    name: 'target_rkt', 
            ...
            drawCallback: function(settings) {
                ...
                api.column(1, { page: 'current' }).data().each(function(group, i) {
                    if (last !== group) {
                        $(rows).eq(i).before(
                            '<tr class="group bg-light"><td colspan="7" class="fw-bolder text-gray-800 fs-6 py-3">Sub Kegiatan: ' + group + '</td></tr>'
                        );
                        last = group;
                    }
                });
    ...
    function editRkt(id) {
        $.ajax({
            url: "{{ url('frontend/rkt/subkegiatan') }}/" + id + "/edit",
            type: "GET",
            success: function(data) {
                $('#edit_id').val(data.refindikatorsubkegiatan_id);
                
                let sasaran = data.cascading.uraian_sasaransubkegiatan ?? '';
                let indikator = data.cascading.uraian_indikatorsubkegiatan ?? '';
                $('#edit_indikator').val((sasaran ? sasaran + ' / ' : '') + indikator);
                
                $('#edit_subkegiatan').val(data.subkegiatan ? data.subkegiatan.nama_subkegiatan : '-');
    ...
    $('#form_edit_rkt').submit(function(e) {
        ...
        $.ajax({
            url: "{{ url('frontend/rkt/subkegiatan') }}/" + id,
            type: "PUT",
            data: {
                _token: "{{ csrf_token() }}",
